...
class DigitalTwinPredictor:
    ...
    def draw_predictions(self):
        ...
        for vid, data in self.active_predictions.items():
            ...
            try:
                ...
                # Start lines ahead of front bumper
                front_offset = vehicle_length / 2.0 + 1.0  # Front + 3m buffer
                lateral_offset = 0.8  # Distance between lines
                
                # Calculate direction vector (forward for this vehicle)
                # Use SUMO angle directly - it always points in the vehicle's forward direction
                dx = np.sin(angle_rad)
                dy = np.cos(angle_rad)
                ...
                # Starting point: front bumper position
                start_x = current_pos[0] + dx * front_offset
                start_y = current_pos[1] + dy * front_offset
                
                # Build PREDICTION line in absolute coordinates (green, upper line)
                pred_points_abs = []
                for i in range(use_len):
                    dt = (i + 1) * 0.25  # 4Hz sampling
                    distance = speed * dt
                    
                    # Point along trajectory + lateral offset
                    px = start_x + dx * distance + perp_dx * lateral_offset
                    py = start_y + dy * distance + perp_dy * lateral_offset
                    pred_points_abs.append((px, py))
                
                # Draw prediction line (green)
                if len(pred_points_abs) > 1:
                ...
                # Build GROUND TRUTH line in absolute coordinates (red, lower line)
                if self.config.DRAW_GROUND_TRUTH:
                    try:
                        ...
                        for i in range(use_len):
                            ...
                            # Point along trajectory - lateral offset (opposite side)
                            gx = start_x + dx * distance - perp_dx * lateral_offset
                            gy = start_y + dy * distance - perp_dy * lateral_offset
                            gt_points_abs.append((gx, gy))
                        
                        if len(gt_points_abs) > 1:
                            self._draw_dashed_line_absolute(
                                vid, "gt",
                                gt_points_abs,
    ...
    def _draw_dashed_line_absolute(self, vehicle_id, prefix, points, color):
        """Draw a dashed line from absolute world coordinates"""
        if len(points) < 2:
            return
    ...
    #             # Calculate direction vector (forward for this vehicle)
    #             if is_leftward:
    #                 # Leftward traffic (moving in negative x direction in SUMO)
    #                 dx = -np.cos(np.radians(90 - angle))
    #                 dy = -np.sin(np.radians(90 - angle))
    #             else:
    #                 # Rightward traffic  
    #                 dx = np.cos(np.radians(90 - angle))
    #                 dy = np.sin(np.radians(90 - angle))
